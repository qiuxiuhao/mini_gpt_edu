"""Stage 6 新增：train the complete Decoder-only GPT language model."""
...
def count_parameters(model: torch.nn.Module) -> int:
    """Stage 6 新增：count all model parameters."""
    return sum(parameter.numel() for parameter in model.parameters())


def main() -> None:
    """Stage 6 新增：run GPT training from CLI arguments."""
    parser = argparse.ArgumentParser(description="Train a Decoder-only GPT model.")
