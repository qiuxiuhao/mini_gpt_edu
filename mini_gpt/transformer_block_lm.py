"""Stage 5 新增：language model with one Transformer Block.

This teaching model adds a single pre-LN Transformer Block after token and
position embeddings. It intentionally does not implement a full Decoder-only
GPT, SFT, LoRA, or RAG.
"""
...
class TransformerBlockLanguageModel(nn.Module):
    """Stage 5 新增：Embedding LM plus one pre-LN Transformer Block."""
    ...
    def __init__(
        ...
        if n_layer != 1:
            raise ValueError(
                "Stage 5 只实现单层 Transformer Block。"
                f"当前 n_layer={n_layer}，多层堆叠会在 Stage 6 实现。"
            )
        if n_embd % n_head != 0:
            raise ValueError(
    ...
    def forward(
        ...
        """Stage 5 新增：run Transformer Block LM and optionally return attention.
        ...
        # x shape after Transformer Block: [batch_size, block_size, n_embd]
        x, attention_weights = self.block(x, return_attention=return_attention)
    ...
    def generate(self, idx: torch.Tensor, max_new_tokens: int) -> torch.Tensor:
        """Stage 5 新增：generate token ids autoregressively.
