"""Stage 5 新增：visualize Transformer Block attention weights as heatmaps."""
...
from mini_gpt.transformer_block_lm import TransformerBlockLanguageModel
...
def save_head_heatmap(
    ...
    """Stage 5 新增：save one Transformer Block attention head heatmap."""
    ...
    ax.set_title(f"Transformer Block Head {head_index} Attention")
...
def main() -> None:
    """Stage 5 新增：load a checkpoint and save per-head attention heatmaps."""
    parser = argparse.ArgumentParser(
        description="Visualize one-block Transformer attention weights."
    )
    parser.add_argument("--ckpt", required=True, help="Path to model checkpoint.")
    parser.add_argument("--prompt", required=True, help="Prompt text.")
    parser.add_argument(
        "--output-dir",
        default="outputs/transformer_block_attention",
